...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
        source=ROOT / 'process/buffer/source',  # file/dir/URL/glob, 0 for webcam
    ...
    save_dir = ROOT / 'process/buffer/detect_result'
    save_root_dir = ROOT / 'process/buffer'
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            #自己改的
            location_center_dir = str(save_root_dir)+ '/detect_location'
            # print(location_center_dir)
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    x0 = format(x0/img_w, '.6f')  #此处我要保存和训练标注的txt一样的格式，故保留六位小数
                    y0 = format(y0/img_h, '.6f')
                    chang = format(chang/img_h, '.6f') # 2448和2048为我的图片长和宽
                    kuan = format(kuan/img_w, '.6f')
                    print("----")
...
def parse_opt():
    ...
    # parser.add_argument('--data', type=str, default=ROOT / 'C:/Users/13291/Downloads/yolov5-master/data/mydata.yaml', help='data.yaml path(s)')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'best.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'process/buffer/source', help='file/dir/URL/glob, 0 for webcam')
